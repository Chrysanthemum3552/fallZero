...
class ExamFragment : Fragment(), PoseLandmarkerHelper.LandmarkerListener {
                ...
                val analyzer = ImageAnalysis.Builder()
                    ...
                    .build().also { ia ->
                        ia.setAnalyzer(cameraExecutor!!) { proxy ->
                            poseLandmarkerHelper?.detectLiveStream(proxy, isFrontCamera = isFrontCamera) ?: proxy.close()
                        }
                    }
                cameraPreview = preview; cameraAnalyzer = analyzer
                bindCameraToSelector(provider)
                autoStartExam()
            } catch (e: Exception) { Log.e(TAG, "Camera binding failed", e); _binding?.tvExamPhase?.text = "카메라 오류: ${e.message}" }
    ...
    private fun autoStartExam() {
        when (mode) {
            Mode.BALANCE -> {
                ...
                if (singleStage in 1..4) { SessionFlow.singleBalanceStage = 0; viewModel.startSingleBalanceStage(singleStage) }
                else viewModel.startBalanceFlow()
            }
            Mode.CHAIR_STAND -> {
                chairPrepareMode = true; chairPreparePhase = 0; bodyStillSinceMs = 0L
                lastShoulderY = 0f; standingMSamples.clear()
    ...
    // ─── 수정됨: 영상에 정지 구간 내장 → TTS만 타이밍에 맞춰 재생 (실측값 기준) ───
    ...
    private fun startBarSimulation(onAfterScript: () -> Unit) {
        ...
        val barGuide = { p: Float -> com.fallzero.app.ui.overlay.ExerciseGuide.Bar(
    ...
    private fun showExamGuidance(titleRes: Int, scriptRes: Int, onAfterScript: () -> Unit) {
        val b = _binding ?: return
        b.tvGuidanceTitle.text = getString(titleRes); b.tvGuidanceText.text = getString(scriptRes)
        b.tvGuidanceCountdown.visibility = View.GONE; b.guidanceOverlay.visibility = View.VISIBLE
        ttsManager?.speak(getString(scriptRes).replace("\n", " ")) {
            if (_binding == null || hasNavigated) return@speak
            _binding?.guidanceOverlay?.visibility = View.GONE; onAfterScript()
        }
    }

    private fun balanceTitleRes(stage: Int) = when (stage) {
        1->R.string.exam_guide_balance_1_title; 2->R.string.exam_guide_balance_2_title
        3->R.string.exam_guide_balance_3_title; 4->R.string.exam_guide_balance_4_title
        else->R.string.exam_guide_balance_1_title }

    private fun balanceScriptRes(stage: Int) = when (stage) {
        1->R.string.exam_guide_balance_1; 2->R.string.exam_guide_balance_2
        3->R.string.exam_guide_balance_3; 4->R.string.exam_guide_balance_4
                    ...
                    when (phase) {
                        is ExamViewModel.ExamPhase.ChairStand -> {
                            ...
                            if (phase.errorHint != null) {
                                ...
                                if (now - lastHintSpokenMs > 4000L) { lastHintSpokenMs = now; ttsManager?.speak(phase.errorHint) }
                            }
                        }
                        is ExamViewModel.ExamPhase.ChairStandComplete -> {
                            ...
                            ttsManager?.speak("의자 일어서기 검사가 끝났어요.") { if (_binding != null && !hasNavigated) navigateNext() }
                        }
                        is ExamViewModel.ExamPhase.BalancePrepare -> {
                            b.layoutCornerCount.visibility = View.GONE; hideFloatingFeedback(); stopChairGuideVideo()
                            b.tvExamPhase.text = "${phase.stage}단계: ${phase.stageName}"
                            b.tvTimer.text = ""; b.tvCount.text = ""; lastSpokenSecond = -1; lastHintText = null
                            showBalanceGuidance(phase.stage, phase.stageName) {
                                ...
                                startCountdown321 {
                                    if (_binding != null && !hasNavigated) {
                                        ...
                                        viewModel.startBalanceMeasurementNow(); startUserAwayMonitor()
                                    }
                                }
                            }
                        }
                        is ExamViewModel.ExamPhase.Balance -> {
                            ...
                            if (phase.isStable) {
                                showFloatingFeedback("✓ 잘 하고 있어요!", 0xFF4CAF50.toInt()); b.tvCount.text = ""
                                ...
                                if (elapsed > 0 && elapsed != lastSpokenSecond) {
                                    ...
                                    if (ttsManager?.isSpeaking() != true) {
                                        ttsManager?.speak(when(elapsed){1->"일";2->"이";3->"삼";4->"사";5->"오";6->"육";7->"칠";8->"팔";9->"구";10->"십";else->elapsed.toString()})
                                    }
                                }
                            } else {
                                val hint = phase.errorHint
                                showFloatingFeedback(hint ?: "자세를 유지하세요", 0xFFFFEB3B.toInt()); b.tvCount.text = ""; lastSpokenSecond = -1
                                if (hint != null && hint != "균형을 잡아주세요") {
                                    val now = System.currentTimeMillis()
                                    if (now - lastHintSpokenMs > 4000L) { lastHintSpokenMs = now; lastHintText = hint; ttsManager?.speak(hint) }
                                }
                            }
                        }
                        is ExamViewModel.ExamPhase.BalanceStagePassed -> {
                            ...
                            ttsManager?.speakSequence("십!", "좋아요!") { if (_binding != null && !hasNavigated) viewModel.advanceFromStagePassed() }
                        }
                        is ExamViewModel.ExamPhase.BalanceComplete -> {
                            ...
                            ttsManager?.speak("균형 검사가 끝났습니다! 수고하셨어요.") { if (_binding != null && !hasNavigated) navigateNext() }
                        }
                        is ExamViewModel.ExamPhase.Completed, ExamViewModel.ExamPhase.Idle -> {}
                    }
                }
            }
        }
    ...
    private fun updateExamGuide(landmarks: List<com.google.mediapipe.tasks.components.containers.NormalizedLandmark>) {
        ...
        when (val guide = viewModel.getGuide(landmarks)) {
            is com.fallzero.app.ui.overlay.ExerciseGuide.Bar -> { b.guideBar.visibility=View.VISIBLE; b.guideBubble.visibility=View.GONE; b.guideBar.setGuide(guide) }
            is com.fallzero.app.ui.overlay.ExerciseGuide.Bubble -> { b.guideBar.visibility=View.GONE; b.guideBubble.visibility=View.VISIBLE; b.guideBubble.setGuide(guide) }
            null -> hideExamGuides()
        }
    }
    ...
    // ─── 수정됨: 단계별 영상 + 실측 TTS 타이밍 ───
    private fun playBalanceGuidanceVideo(stage: Int, stageName: String, onAfter: () -> Unit) {
        val b = _binding ?: return
        b.videoChairGuidance.setZOrderOnTop(true); b.videoChairGuidance.visibility = View.VISIBLE
        b.tvChairSubtitle.visibility = View.GONE  // 자막은 영상에 포함됨
        val videoRes = when (stage) { 1->R.raw.balance_guide_1; 2->R.raw.balance_guide_2; 3->R.raw.balance_guide_3; 4->R.raw.balance_guide_4; else->R.raw.balance_guide_1 }
        val uri = Uri.parse("android.resource://${requireContext().packageName}/${videoRes}")
        b.videoChairGuidance.setOnPreparedListener { mp ->
            mp.isLooping = false; mp.setVolume(0f, 0f)
            adjustVideoToAspectRatio(mp.videoWidth, mp.videoHeight); mp.start()
            when (stage) {
                1 -> {
                    // 0.00초: "뒷꿈치를 모아주시고, 팔을 가슴 앞에 교차하세요" (정지 4.88초)
                    ttsManager?.speak("뒷꿈치를 모아주시고, 팔을 가슴 앞에 교차하세요")
                    // 6.88초: "자세를 유지해주세요" (정지 2.01초)
                    b.root.postDelayed({ if (_binding==null||hasNavigated) return@postDelayed; ttsManager?.speak("자세를 유지해주세요") }, 6880L)
                }
                2 -> {
                    // 0.00초: "두 팔은 가슴에 교차해주세요" (정지 2.50초)
                    ttsManager?.speak("두 팔은 가슴에 교차해주세요")
                    // TODO: 영상 완성 후 실측값으로 조정
                    b.root.postDelayed({ if (_binding==null||hasNavigated) return@postDelayed; ttsManager?.speak("한쪽 발 뒤꿈치를 다른 발 엄지발가락 옆에 놓으세요") }, 4500L)
                }
                3 -> {
                    // 0.00초: "두 팔은 가슴에 교차해주세요" (정지 2.50초)
                    ttsManager?.speak("두 팔은 가슴에 교차해주세요")
                    // TODO: 영상 완성 후 실측값으로 조정
                    b.root.postDelayed({ if (_binding==null||hasNavigated) return@postDelayed; ttsManager?.speak("한쪽 발 뒤꿈치를 다른 발 발끝 바로 앞에 일렬로 놓으세요") }, 4500L)
                }
                4 -> {
                    // 0.00초: "두 팔은 가슴에 교차해주세요" (정지 2.50초)
                    ttsManager?.speak("두 팔은 가슴에 교차해주세요")
                    // 5.00초: "한쪽 발을 들어 올려 한 발로만 서주세요" (정지 3.54초)
                    b.root.postDelayed({ if (_binding==null||hasNavigated) return@postDelayed; ttsManager?.speak("한쪽 발을 들어 올려 한 발로만 서주세요") }, 5000L)
                    // 12.07초: "자세를 유지해주세요" (정지 2.01초)
                    b.root.postDelayed({ if (_binding==null||hasNavigated) return@postDelayed; ttsManager?.speak("자세를 유지해주세요") }, 12070L)
                }
            }
        }
        b.videoChairGuidance.setOnCompletionListener {
            if (_binding==null||hasNavigated) return@setOnCompletionListener
            _binding?.videoChairGuidance?.visibility=View.GONE; _binding?.guidanceOverlay?.visibility=View.GONE; _binding?.vCountdownDim?.visibility=View.VISIBLE
            if (stage==1) startRingExplanation(stage, stageName, onAfter) else showBalanceHoldCallout(stage, stageName, onAfter)
        }
        b.videoChairGuidance.setVideoURI(uri)
    }
    ...
    private fun startRingExplanation(stage: Int, stageName: String, onAfter: () -> Unit) {
        val b = _binding ?: return
        isRingExplaining = true; b.guideBubble.visibility=View.VISIBLE; b.guideBubble.bringToFront()
        val mkBubble = { hold: Float -> com.fallzero.app.ui.overlay.ExerciseGuide.Bubble(swayRatio=0f,holdProgress=hold,label="",elapsedSec=hold*10f,targetSec=10f,poseValid=true) }
        ...
        ttsManager?.speak("자세를 잘 잡으면 원이 시계방향으로 부드럽게 채워집니다") {
            ...
            animateRing(0f,1f,0f,3000L,{_,h->mkBubble(h)}) {
                ...
                ttsManager?.speak("자세가 틀리면 0초부터 다시 시작합니다") {
                    ...
                    _binding?.root?.postDelayed({ if (_binding==null||hasNavigated) return@postDelayed; isRingExplaining=false; _binding?.guideBubble?.visibility=View.GONE; showBalanceHoldCallout(stage,stageName,onAfter) }, 1000L)
                }
            }
        }
    }
    ...
    private fun startRingSimulation(stage: Int, stageName: String, onAfter: () -> Unit) {
        val b = _binding ?: return
        isRingExplaining=true; b.guideBubble.visibility=View.VISIBLE; b.guideBubble.bringToFront()
        b.guideBubble.setGuide(com.fallzero.app.ui.overlay.ExerciseGuide.Bubble(0f,0f,stageName,0f,10f,true)); b.guideBubble.invalidate()
        ttsManager?.speak("제대로 된 자세를 하면 원의 테두리가 초록색이 되며 시계방향으로 채워집니다")
        android.animation.ValueAnimator.ofFloat(0f,1f).apply {
            duration=5000L
            addUpdateListener { val p=it.animatedValue as Float; _binding?.guideBubble?.setGuide(com.fallzero.app.ui.overlay.ExerciseGuide.Bubble(0f,p,stageName,p*10f,10f,true)) }
            addListener(object:android.animation.AnimatorListenerAdapter(){ override fun onAnimationEnd(a:android.animation.Animator){ if(_binding==null||hasNavigated)return; isRingExplaining=false; _binding?.guideBubble?.visibility=View.GONE; showBalanceHoldCallout(stage,stageName,onAfter) } })
        }.start()
    }
    ...
    private fun showBalanceHoldCallout(stage: Int, stageName: String, onAfter: () -> Unit) {
        ...
        b.videoChairGuidance.visibility=View.GONE; b.ivChairFrontPose.visibility=View.GONE; b.tvChairSubtitle.visibility=View.GONE
        b.tvVideoPlaceholder.text="10초간\n유지!"; b.tvVideoPlaceholder.textSize=64f; b.tvVideoPlaceholder.setTextColor(0xFFFFFF00.toInt()); b.tvVideoPlaceholder.visibility=View.VISIBLE
        b.guidanceOverlay.visibility=View.VISIBLE
        ttsManager?.speak("$stageName 자세를 10초간 유지해주세요") {
            if (_binding==null||hasNavigated) return@speak
            _binding?.tvVideoPlaceholder?.visibility=View.GONE; _binding?.guidanceOverlay?.visibility=View.GONE; _binding?.vCountdownDim?.visibility=View.VISIBLE; onAfter()
        }
    }
    ...
    private fun showThirtySecCallout(onAfter: () -> Unit) {
        ...
        b.tvVideoPlaceholder.text="30초간\n최대한 많이!"; b.tvVideoPlaceholder.textSize=56f; b.tvVideoPlaceholder.setTextColor(0xFFFFFF00.toInt()); b.tvVideoPlaceholder.visibility=View.VISIBLE
        b.guidanceOverlay.visibility=View.VISIBLE
        ttsManager?.speak("이 자세를 30초간 최대한 많이 반복하시면 됩니다") {
            if (_binding==null||hasNavigated) return@speak
            _binding?.tvVideoPlaceholder?.visibility=View.GONE; _binding?.guidanceOverlay?.visibility=View.GONE; onAfter()
        }
    }
    ...
    private fun adjustVideoToAspectRatio(videoW: Int, videoH: Int) {
        ...
        view.post {
            ...
            val pw=parent.width; val ph=(((_binding?.layoutGuidanceText?.top?:parent.height)-(_binding?.tvGuidanceTitle?.bottom?:0)-24).coerceAtLeast(0))
            if (pw<=0||ph<=0) return@post
            val vr=videoW.toFloat()/videoH; val pr=pw.toFloat()/ph
            val params=v.layoutParams
            if (vr>pr){params.width=pw;params.height=(pw/vr).toInt()} else {params.height=ph;params.width=(ph*vr).toInt()}
            v.layoutParams=params
        }
    }

    private fun showFloatingFeedback(msg: String, colorArgb: Int) {
        val b = _binding ?: return; b.tvFloatingFeedback.text=msg; b.tvFloatingFeedback.setTextColor(colorArgb); b.tvFloatingFeedback.visibility=View.VISIBLE
    }
    ...
    private fun navigateNext() {
        if (hasNavigated) return; hasNavigated=true; cleanupCamera()
        ...
        if (next.type==SessionFlow.StepType.DONE) {
            ...
            b.tvExamPhase.text=""; b.tvTimer.text=""; b.tvCount.text="✔ 끝!"; b.tvCount.setTextColor(0xFF4CAF50.toInt()); b.poseOverlay.clear()
            ttsManager?.speak("모든 검사가 끝났습니다.") { if (_binding!=null) findNavController().navigate(R.id.action_global_exam_result) }
            return
        }
        navigateTo(next)
    }
    ...
    private fun bindCameraToSelector(provider: ProcessCameraProvider) {
        ...
        try { provider.unbindAll(); val preview=cameraPreview?:return; val analyzer=cameraAnalyzer?:return; provider.bindToLifecycle(viewLifecycleOwner,selector,preview,analyzer) }
        catch (e: Exception) { Log.e(TAG,"bindCameraToSelector failed",e) }
    }

    private fun toggleCameraFacing() {
        isFrontCamera=!isFrontCamera; com.fallzero.app.util.CameraFacingPref.setFrontCamera(requireContext(),isFrontCamera); cameraProvider?.let { bindCameraToSelector(it) }
    }

    private fun cleanupCamera() {
        try { poseLandmarkerHelper?.clearPoseLandmarker(); poseLandmarkerHelper=null; cameraProvider?.unbindAll(); cameraProvider=null }
    ...
    override fun onResults(resultBundle: PoseLandmarkerHelper.ResultBundle) {
        ...
        if (landmarks != null) {
            ...
            if (isPausedForUserAway) onUserReturned() else if (isPausedForOcclusion&&fullBody) onOcclusionCleared()
        }
        activity?.runOnUiThread {
            val b = _binding ?: return@runOnUiThread
            if (!isAdded||hasNavigated) return@runOnUiThread
            if (landmarks!=null) b.poseOverlay.setResults(result,resultBundle.inputImageHeight,resultBundle.inputImageWidth)
            if (chairPrepareMode) {
                ...
                if (chairPreparePhase==1&&landmarks!=null&&landmarks.size>=33) {
                    ...
                    val ankleY=maxOf(landmarks[27].y(),landmarks[28].y()); val m=(ankleY-shoulderY)*100f
                    val yChange=if(lastShoulderY>0f)abs(shoulderY-lastShoulderY) else 1f; lastShoulderY=shoulderY
                    ...
                    if (isStill) {
                        ...
                        standingMSamples.add(m); val held=System.currentTimeMillis()-bodyStillSinceMs
                        if (held>=3000L) {
                            chairPreparePhase=2
                            val avgM=if(standingMSamples.isNotEmpty())standingMSamples.average().toFloat() else 50f
                            viewModel.calibrateChairStand(avgM)
                            showFloatingFeedback("✓ 준비 완료",0xFF4CAF50.toInt()); b.tvCount.text=""; b.tvTimer.text=""
                                    ...
                                    ttsManager?.speak("이제 시작하겠습니다") {
                                        ...
                                        startCountdown321 {
                                            if (_binding!=null&&!hasNavigated&&chairPrepareMode) {
                                                ...
                                                viewModel.startChairStand(); _binding?.guideBar?.bringToFront(); startUserAwayMonitor()
                                            }
                                        }
                                    }
                                }
                            }
                        } else { val remain=((3000L-held)/1000)+1; b.tvTimer.text="감지 중... ${remain}초" }
                    } else { bodyStillSinceMs=0L; standingMSamples.clear(); b.tvTimer.text="의자 앞에 가만히 서주세요" }
                }
                return@runOnUiThread
            }
            if (landmarks!=null) {
                viewModel.processLandmarks(landmarks)
                if (!isRingExplaining) { if (showGuide) updateExamGuide(landmarks) else hideExamGuides() }
            }
        }
    }
    ...
    private fun startCountdown321(onReady: () -> Unit) {
        ...
        b.tvBigCountdown.setTextColor(0xFFFFFF00.toInt()); b.tvBigCountdown.text="3"; ttsManager?.speak("삼")
        b.root.postDelayed({
            if (_binding==null||hasNavigated) return@postDelayed
            ...
            _binding?.root?.postDelayed({
                ...
                _binding?.root?.postDelayed({
                    ...
                    ttsManager?.speak("시작!") { _binding?.tvBigCountdown?.visibility=View.GONE; _binding?.vCountdownDim?.visibility=View.GONE; if (_binding!=null&&!hasNavigated) onReady() }
                },1000L)
            },1000L)
        },1000L)
    }
    ...
    private fun startUserAwayMonitor() {
        userAwayCheckJob?.cancel(); val nowInit=System.currentTimeMillis(); lastValidFrameMs=nowInit; lastFullBodyMs=nowInit
        userAwayCheckJob=viewLifecycleOwner.lifecycleScope.launch {
            while (isAdded&&!hasNavigated) {
                delay(500); if (isPausedForUserAway||isPausedForOcclusion) continue
                val now=System.currentTimeMillis()
                if (lastValidFrameMs>0L&&now-lastValidFrameMs>USER_AWAY_TIMEOUT_MS) onUserAway()
                else if (lastFullBodyMs>0L&&now-lastFullBodyMs>USER_AWAY_TIMEOUT_MS) onPartialOcclusion()
            }
        }
    }
    ...
    private fun onUserAway() {
        ...
        Log.d(TAG,"★ onUserAway TRIGGERED — gap=${System.currentTimeMillis()-lastValidFrameMs}ms")
        isPausedForUserAway=true; viewModel.pauseForUserAway(); showExamPauseOverlay(EXAM_USER_AWAY_MSG)
    }

    private fun onPartialOcclusion() {
        if (isPausedForUserAway||isPausedForOcclusion) return
        Log.d(TAG,"★ onPartialOcclusion TRIGGERED — gap=${System.currentTimeMillis()-lastFullBodyMs}ms")
        isPausedForOcclusion=true; viewModel.pauseForUserAway(); showExamPauseOverlay(EXAM_OCCLUSION_MSG)
    }

    private fun showExamPauseOverlay(msg: String) {
        val b=_binding?:return; b.pauseOverlay.visibility=View.VISIBLE; b.tvPauseMessage.text=msg; ttsManager?.speak(msg)
        pauseAnnounceJob?.cancel()
        pauseAnnounceJob=viewLifecycleOwner.lifecycleScope.launch {
            while ((isPausedForUserAway||isPausedForOcclusion)&&isAdded&&!hasNavigated) { delay(PAUSE_TTS_LOOP_MS); if (isPausedForUserAway||isPausedForOcclusion) ttsManager?.speak(msg) }
        }
    }
    ...
    private fun onOcclusionCleared() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            delay(PAUSE_RESUME_BUFFER_MS); userReturnInProgress=false; if (!isAdded||hasNavigated) return@launch
            _binding?.pauseOverlay?.visibility=View.GONE; isPausedForOcclusion=false; viewModel.resumeFromUserAway(); ttsManager?.speak("전신이 잘 보입니다. 다시 시작합니다.")
        }
    }
    ...
    private fun onUserReturned() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            delay(PAUSE_RESUME_BUFFER_MS); userReturnInProgress=false; if (!isAdded||hasNavigated) return@launch
            _binding?.pauseOverlay?.visibility=View.GONE; isPausedForUserAway=false; viewModel.resumeFromUserAway(); ttsManager?.speak("다시 시작합니다.")
        }
    }
    ...
    private fun isFullBodyVisible(landmarks: List<com.google.mediapipe.tasks.components.containers.NormalizedLandmark>?): Boolean {
        ...
        val noseY=landmarks[0].y(); val ankleY=maxOf(landmarks[27].y(),landmarks[28].y()); val span=ankleY-noseY
        if (span<0.30f||span>0.97f||noseY<0.02f) return false
        return true
    }

    override fun onError(error: String, errorCode: Int) { activity?.runOnUiThread { _binding?.tvExamPhase?.text="오류: $error" } }

    override fun onDestroyView() {
        super.onDestroyView()
        userAwayCheckJob?.cancel(); userAwayCheckJob=null; pauseAnnounceJob?.cancel(); pauseAnnounceJob=null
        cleanupCamera()
